...
psihat <- function(Y, lab, alpha){
    ...
    C = Cplus + Cminus
    return(list(eigen(C)$vectors[,1], eigen(C)$values[1]))
    #return(eigen(C)$values) 
}
...
#        print(head(psi))
    ...
    return(list(psi,conv,differ,lab,lambda))
}
...
pec.k <- function(Y, alpha, nk=nk, reset.opt = "random", ini.opt = "mean", lab.ini = NA, iter.tol = 10, reset.tol = 50){
    p = dim(Y)[1]
    n = dim(Y)[2]
    ...
    while(nk > 0){
	  print(paste("computing for k = ", nk))
        ...
        psi<-outp[[1]]; conv=outp[[2]]; lab=outp[[4]]; lambda=outp[[5]]; cconv=c(cconv,conv)
        psi.mat[,nk] <- psi
        lambda.vec[nk]<- lambda
        obs = t(psi)%*%resid.obs
    ...
    #principalcomponent$psi.mat = psi.mat
	return(list(rev(mu.vec), psi.mat, min(cconv),lab,lambda.vec))
}
